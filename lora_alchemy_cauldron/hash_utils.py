...
# config をプロジェクトから読む (ComfyUI側でも読めるように)
try:
    from .config import HASH_METHOD, HASH_CHUNK_SIZE_MB
except ImportError:
    HASH_METHOD = "fast"
...
def compute_lora_id(file_path: Path, method: str = HASH_METHOD, chunk_size_mb: int = HASH_CHUNK_SIZE_MB) -> str:
    """
...
def compute_lora_id_safe(file_path: Path, method: str = HASH_METHOD, fallback_to_stem: bool = True) -> Optional[str]:
    """
